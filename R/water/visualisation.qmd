...
sim_no_water <- read.csv('../../sim/sim_surf_park_no_city_water_high_evap.csv')
sim_city_water <- read.csv('../../sim/sim_surf_park_city_water_surfers_high_evap_high_surfers.csv')
```
...
n <-  6
volume_bassins <- volume_bassin1+volume_bassin2
...
weekly_no_water %>%
  ggplot() +
  geom_tile(aes(x=weekofyear, y=year, fill=(bassin_volume/(volume_bassins)*100)), colour="white", size=0.2)+
...
weekly_no_water$capacity_f <- factor(weekly_no_water$capacity, labels = c("Aucun bassin", "Petit bassin", "Grand bassin", "Deux bassins"))
weekly_no_water %>%
  ggplot() +
